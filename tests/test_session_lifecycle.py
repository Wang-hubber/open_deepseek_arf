...
class TestStage5SessionEnd:
    ...
    def test_list_archives_newest_first(self, tmp_path):
        """list_archives returns newest first, without message bodies."""
        from arf.server.sessions import list_archives

        ws = _make_workspace(tmp_path)
        _write_archive(ws, "20260518_100000", [{"role": "user", "content": "a"}],
                       created_at="2026-05-18T10:00:00+00:00")
        _write_archive(ws, "20260518_100001", [{"role": "user", "content": "b"}],
                       created_at="2026-05-18T10:00:01+00:00")

        archives = list_archives(str(ws))
        assert len(archives) == 2
        assert archives[0]["id"] == "20260518_100001"
        assert "messages" not in archives[0]

    # ---- update_title / delete_archive ----------------------------------

    def test_update_title_in_archive(self, tmp_path):
        from arf.server.sessions import update_title

        ws = _make_workspace(tmp_path)
        sid = _write_archive(ws, "20260518_100000",
    ...
    def test_delete_archive(self, tmp_path):
        from arf.server.sessions import delete_archive
    ...
    def test_resume_archive_loads_messages_and_sets_title(self, tmp_path):
        """Resume loads archived messages into a new session with correct metadata."""
        from arf.server.session_manager import SessionManager
        from arf.server.sessions import get_archive

        ws = _make_workspace(tmp_path)

        # 1. Create and archive an old session
        old_history = [
            {"role": "user", "content": "hello"},
        ...
        old_sid = _write_archive(ws, "20260518_100000", old_history, title="Weather Chat")

        # 2. Simulate resume: create a new SessionManager and load the archive
        mgr = SessionManager(ws)
        archive = get_archive(old_sid, str(ws))
        ...
        # 3. Verify state
        assert mgr.session_title == "Weather Chat"
        assert mgr.needs_title is False
        assert len(mgr.session_history) == 4
        assert mgr.session_history[0]["role"] == "user"
        assert mgr.session_history[0]["content"] == "hello"

    def test_resume_preserves_archive_on_disk(self, tmp_path):
        """Resume keeps the archived file intact — it is not deleted after resume."""
        from arf.server.session_manager import SessionManager
        from arf.server.sessions import get_archive
        ...
        mgr = SessionManager(ws)
        mgr.reset_session_history(title=archive["title"])
        mgr.session_history = list(archive["messages"])
        mgr.needs_title = False

        # Archive file still exists after resume
        assert archive_path.exists()
        data = json.loads(archive_path.read_text(encoding="utf-8"))
        assert data["id"] == old_sid
...
class TestFullLifecycle:
    ...
    def test_session_end_to_archive_flow(self, tmp_path):
        """Session history accumulates, fire_session_end persists via hook."""
        from arf.server.session_manager import SessionManager
        ...
        # fire_session_end triggers the session_archiver hook
        mgr.fire_session_end()

        # History is preserved after fire_session_end
        assert len(mgr.session_history) == 4
        assert mgr.session_history[0]["content"] == "你好"
