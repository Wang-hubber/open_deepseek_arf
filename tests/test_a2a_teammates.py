...
class TestSendPeerMessage:
    ...
    @pytest.mark.anyio
    async def test_send_peer_message_registers_wait(self):
        """send_peer_message registers before_round wait when _register_wait is provided."""
        from arf.plugins.a2a_teammates.state import register_bus as _register_bus
        from arf.plugins.a2a_teammates.tools.send_peer_message.function import execute

        # Register both the bus (for get_bus lookup) and the agent (for inbox delivery)
        _register_bus("default_group__dev", _state_mod._state.agent_bus)
        await _state_mod._state.agent_bus.register(
            AgentInfo(name="default_group__dev", description="", capabilities=[]))

        register_calls = []

        def fake_register(hook_name, reason, resume_key):
            register_calls.append((hook_name, reason, resume_key))
            return type("WI", (), {"wait_id": "wi_peer_test"})()

        result = await execute(
            to="default_group__dev",
            message="task",
            session_id="default_group__pm",
            _register_wait=fake_register,
        )
        assert result["ok"] is True
        assert result["wait_id"] == "wi_peer_test"
        assert len(register_calls) == 1
        assert register_calls[0][0] == "before_round"
        assert "peer_wait:" in register_calls[0][1]
