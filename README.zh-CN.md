...
| 要解决的问题 | OS 经典方案 | ARF 当前实现 | 演进方向 |
|-------------|------------|-------------|----------|
| **上下文窗口耗尽（OOM）** | 虚拟内存 + 页交换：冷页换出到磁盘，按需换入 | 上下文压缩器：75% 水位触发滑动窗口摘要；对话历史换出到 `archive.json` | 细粒度"缺页"调入：按最小语义单元（函数签名、关键决策）精确检索注入，而非整块解压；LRU 相关性淘汰 |
| **长程记忆与状态持久化** | 文件系统：数据以文件形式持久存储，按路径索引 | 三层记忆体系：`session.md`（短期）、`long_term.md`（画像/事实）、`sessions/*.json`（归档）；`memory_extractor` Hook 自动提取 | 语义文件系统：Agent 自动维护知识图谱式索引；记忆可按语义路径访问（"用户对 Python 类型标注的偏好"），而非字面路径 |
| **快慢推理调度** | 多级缓存：L1 快而小，L2 慢而大；CPU 自动判断命中 | 二级分类器：`quick_thinking` ↔ `deep_thinking` 自动路由；`quick_no_thinking` 保留给后台任务；模型不可用时自动降级 | 硬件感知的动态推理缓存：任务复杂度 × 延迟预算 × KV Cache 占用 → 自动选择 L1/L2/L3 推理；同会话内无感切换模型，支持上下文"进程迁移" |
| **模型计算资源分配** | big.LITTLE 异构调度：轻任务用小核，重任务用大核 | 可配置 max_turns；模型按优先级解析（`quick_thinking` → `deep_thinking` → 任意已配置）；工具级渐进式披露限制单次调用算力消耗 | 自适应模型"核"调度器：实时监测任务难度；同会话内无感切换模型；弱模型群体协作可超越单强模型 |
| **外部能力调用** | 系统调用 + 协处理器：CPU 通过中断和 I/O 指令调用 NPU、GPU、磁盘 | 工具调用：每工具 `tool.yaml` + `function.py`；MCP 协议支持在路线图上；子进程 Hook 作为隔离边界（退出码 0/1/2 契约） | 硬件化工具加速：高频工具（代码解释器、搜索）封装为标准"协处理器"接口；支持并行批处理与 DMA 式异步调用 |
| **任务并行与并发** | 超标量/乱序执行 + 多核：操作数无关的指令同时发射 | `SysAgent` 子代理派生；Worktree 隔离独立子任务；Hook 线程池并行执行 | 多 Agent 流水线优化：自动分析任务依赖图；动态并行度调节；Agent 间流水线深度与吞吐优化 |
| **外部中断与用户干预** | 硬件中断：保存现场 → 执行 ISR → 恢复现场 | 异步双缓冲：用户可通过 SSE 在 Agent 工作途中注入新指令；会话空闲超时暂停；Hook 注入退出码（2） | Agent 信号标准化：定义通用中断向量（暂停/重定向/撤销/注入）；支持键盘、语音等多模态实时打断 |
| **资源死锁与竞争** | 资源分配图 + 死锁检测/规避：锁层级、超时回滚 | 当前为单 Agent 模型；文件编辑通过 `file_writer` 以工作区范围路径串行化 | Agent 间并发控制协议：引入分布式锁管理器（DLM）、乐观并发控制；自动检测并打破 Agent 间的循环等待 |
| **身份、权限与安全边界** | 保护环（Ring 0–3）+ 访问控制列表：内核态拥有最高权限，用户态受限 | 双源隔离：系统资源只读（Ring 0），用户工作区可读写（Ring 3）；工作区路径沙箱（防遍历）；Hook 作为独立子进程运行 | 最小权限自动推导：Agent 仅被授予完成任务所需的最小工具集与数据视图；权限随任务阶段动态伸缩 |
