...
| **Trace 与可观测性** | 系统监控 + 日志：结构化事件日志，实时指标 | 13 种事件类型通过 EventBus → `FileTraceStore`（JSON）+ `UsageTracker`。前端瀑布流按交互轮次分组，可展开：模型响应 → 工具调用 → Hook。资源统计 API。`/trace-viewer` 独立查看器。 [设计文档 →](docs/trace.md) | SQLite Trace 数据库；OpenTelemetry 导出；Trace 搜索/过滤；Span 追踪。 |
