...
|-------------|------------|----------------------|----------|
| **上下文窗口耗尽（OOM）** | 虚拟内存 + 页交换：冷页换出到磁盘，按需换入 | `SlidingWindowCompactor` 每 turn 监测总字符数，超出阈值（1M token 窗口的 75%）时触发。保留最近 4 条消息，旧轮次通过 LLM 摘要存入 `context_summary`。记忆检索结果合并到同一摘要字段。 [设计文档 →](docs/compaction.md) | 细粒度"缺页"调入：按最小语义单元（函数签名、关键决策）精确检索注入，而非整块解压；LRU 相关性淘汰 |
| **长程记忆与状态持久化** | 文件系统：数据以文件形式持久存储，按路径索引 | `FileMemoryStore`（JSON 后端）+ `LLMMemoryWriter`（自动抽取）+ `LLMMemoryRetriever`（语义检索）。每 turn 结束后自动提取记忆——无需显式调用工具。通过 LLM 对比已有条目实现 add/update/delete。 [设计文档 →](docs/memory-pipeline.md) | 语义文件系统：Agent 自动维护知识图谱式索引；记忆可按语义路径访问（"用户对 Python 类型标注的偏好"），而非字面路径 |
