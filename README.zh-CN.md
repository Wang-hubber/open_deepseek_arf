...
| **[工具沙箱 →](docs/tool-sandbox.md)**<br>安全边界 | 系统调用 + 保护环（Ring 0–3）+ ACL | `tool.yaml` + `function.py` 每工具。`PathCheckToolGuard` 递归扫描阻断穿越、symlink、深度/数量配额。权限 deny→ask→allow 管道。 | 每次调用独立沙箱；MCP 协议 |
