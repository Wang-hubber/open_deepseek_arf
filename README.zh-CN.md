...
| **模型路由与资源分配** | 多级缓存 (L1/L2) + big.LITTLE 异构调度 | 二级 LLM 分类器按复杂度路由用户任务（中等→quick，复杂→deep）。专用模型处理框架后台工作（记忆抽取、分类）。每 turn 动态切换。 [设计文档 →](docs/model-routing.md) | 硬件感知调度：任务复杂度 × 延迟预算 × KV Cache 占用；同会话无感切换模型；弱模型协作超越单强模型 |
