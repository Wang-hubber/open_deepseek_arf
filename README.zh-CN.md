...
| **外部能力调用** | 系统调用 + 协处理器：CPU 通过中断和 I/O 指令调用 NPU、GPU、磁盘 | 工具调用：每工具 `tool.yaml`（JSON Schema）+ `function.py`，通过双源注册表加载。子进程 Hook 退出码契约（0=继续，1=阻断，2=注入）。MCP 协议在路线图中。 | 硬件化工具加速：高频工具（代码解释器、搜索）封装为标准"协处理器"接口；支持并行批处理与 DMA 式异步调用 |
...
| **身份、权限与安全边界** | 保护环（Ring 0–3）+ 访问控制列表：内核态拥有最高权限，用户态受限 | 双源隔离：系统资源只读，用户工作区可读写。路径沙箱防止逃逸工作区。Hook 作为独立子进程运行，拥有独立超时和故障域。UserAgent 限制写入 `tools/`、`skills/`、`models/`。 | 最小权限自动推导：Agent 仅被授予完成任务所需的最小工具集与数据视图；权限随任务阶段动态伸缩 |
