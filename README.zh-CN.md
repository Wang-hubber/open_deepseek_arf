...
| **外部中断与用户干预** | 硬件中断：保存现场 → 执行 ISR → 恢复现场 | `cancel_event` 异步取消 + `POST /api/chat/cancel`。3 快照 undo（对话状态 + 工作区文件双回滚）。Hook exit-code-2 消息注入。 [设计文档 →](docs/interrupt.md) | 通用中断向量（暂停/重定向/撤销/注入）；多模态实时打断；会话空闲超时。 |
