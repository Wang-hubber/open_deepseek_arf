...
| 7 | **[安全与沙箱 →](docs/tool-sandbox.md)**<br>访问控制 + 路径安全 | 保护环 (Ring 0-3) + ACL | `PathCheckToolGuard` — 递归扫描（..、符号链接、深度/数量配额）。`DirectoryBoundary` — 两层白名单（全局 `workspace_root` + 工具级 `allowed_dir`）。`SessionModeManager` + `PermissionRegistry` — `session_mode` (auto/ask/plan) 全局模式 + deny→ask→allow 权限列表。`HumanLoop` SSE 推送审批 + 60s 超时。`GuardDefaults` 三道防线（PathCheck/Regex/None）。 | 逐次调用沙箱；MCP 协议；OAuth 范围权限 |
...
| | **安全** | `PathCheckToolGuard`（..、符号链接、深度/数量）、`DirectoryBoundary` 白名单提升、`SessionModeManager`（auto/ask/plan）+ `PermissionRegistry` deny→ask→allow、`HumanLoop` SSE 审批 + 60s 超时、`GuardDefaults` 三道防线 |
...
`SlidingWindowCompactor` 监控上一轮的 token 用量。达到模型上下文窗口 75% 时触发：保留最近 4 条消息，旧轮次通过 LLM 生成 7 维结构化摘要存入 `context_summary`。长工具输出（>2000 字符）落盘，上下文保留摘要指针——无 system model 时静默退化到纯截断。
...
`TwoTierRouter` 通过廉价 LLM 对每次用户查询分类：简单 → `quick`（flash），复杂 → `deep`（pro）。Router/Engine 两层降级链确保每请求都有模型可用。后台任务（压缩、分类、handoff）共用 `system_model`。每轮动态切换。
...
`PathCheckToolGuard` 在每次工具调用前阻断路径穿越和绝对路径。`DirectoryBoundary` 实现两层路径白名单：全局 `workspace_root` 作为所有工具的默认边界，工具可通过 `tool.yaml` 中的 `allowed_dir` 声明更宽的作用域。`SessionModeManager` 解析全局会话模式 (`session_mode`) 与 per-agent policy 生成有效模式，`PermissionRegistry` 据此执行 deny→ask→allow 规则。工具在进程内执行，守卫检查每次调用。
...
| P-11 | ✅ `memory` | DONE | **[长期记忆提取 →](docs/plugins/memory.md)** — 基于 subprocess 的非阻塞提取，通过 sysmodel 驱动。按轮次间隔触发（默认 10 轮）。原子写入 `memory.md`（≤300KB），会话启动时加载。双触发路径：`round_end` hook + `memory_extract` 工具 |
