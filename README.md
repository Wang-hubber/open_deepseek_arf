...
| **[A2A Communication →](docs/a2a-communication.md)**<br>Inter-process communication | Pipes, signals, shared memory, message queues | `HandoffManager` signal-based agent switching integrated into `invoke`/`astream` main loop. `InMemoryAgentBus` — asyncio.Queue-backed message routing (broadcast, targeted send, capability-based discovery). `PeerAgent` — decentralised P2P communication (negotiate, handoff, discover_peers). `DictWorkspace` shared memory, `InMemoryLock` synchronisation, `MajorityVoteConsensus`. Full Protocol layer for AgentBus / Supervisor / Consensus. | Network A2A (gRPC), pub/sub event bus for dynamic agent discovery, DAG-based multi-agent scheduling |
...
| **[Eval Benchmark →](docs/eval-benchmark.md)**<br>Regression detection | CI test suite + session replay | `BenchmarkBuilder` creates test cases from real session traces. `EvalRunner` replays via `agent.chat()`, captures real traces through `EventBus.events_since()`. 4 built-in metrics (success rate, tool accuracy, turn efficiency, output contains). `EvalComparator` diffs reports to detect regressions. | CLI integration; HTML visual report; semantic similarity metric |
| **[API Protection →](docs/api-protection.md)**<br>Rate limiting + fault isolation | I/O throttling + device fencing (cgroup blkio) | Token bucket per-endpoint rate limiting (configurable rps + burst). Exponential cooldown circuit breaker per model (CLOSED→OPEN→HALF_OPEN), trips after `failure_threshold` failures. Events emitted via EventBus → FileTraceStore. Decorator-pattern injection in `_call_model`, zero changes to GraphEngine/ModelAdapter. | Distributed rate limiting (multi-agent quota sharing), adaptive thresholds (history-based failure_threshold), priority queuing (system vs user requests) |
