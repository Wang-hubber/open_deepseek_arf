...
|---------|-------------|---------------|-----------|
| **Memory management (OOM + persistence)** | Virtual memory + file system: page swapping, persistent storage | Token-aware sliding window compaction at 75% threshold with LLM summarization. Long tool outputs written to disk with summary in context. Automatic fact/preference/decision extraction with dedup and semantic retrieval. [Compaction →](docs/compaction.md) [Memory →](docs/memory-pipeline.md) | Fine-grained page fault: semantic-unit retrieval; agent-maintained knowledge graph with path-based + semantic access |
...
| **Tool sandbox & security boundaries** | System calls + protection rings (Ring 0–3) + ACL: kernel gates every syscall, user mode restricted | Tool calling: `tool.yaml` (JSON Schema) + `function.py` per tool. `PathCheckToolGuard` blocks `..` and absolute paths. Dual-source isolation: framework resources read-only, user workspace read-write. Hook exit-code contract (0=continue, 1=block, 2=inject). [Design doc →](docs/tool-sandbox.md) | Per-tool deny→ask→allow permission gating. Isolated sandbox per invocation (CPU, memory, network quotas). Least-privilege auto-derivation. MCP protocol integration. |
| **Concurrency & deadlock prevention** | Superscalar execution + resource allocation graph: dependency analysis, lock hierarchy | Sequential agent execution avoids concurrency. Skills declare tool pipelines with explicit dependencies — engine enforces execution order. Hook parallelization via thread pool. [Design doc →](docs/skill-pipeline.md) | Multi-agent pipeline: automatic DAG analysis; worktree isolation; dynamic parallelism; distributed lock manager. |
| **External interrupt & user intervention** | Hardware interrupt: save state → ISR → restore state | Cancel via `asyncio.Event` + `POST /api/chat/cancel`. 3-snapshot undo (state + files) via API or in-conversation `undo` tool. Hook exit-code-2 message injection. [Design doc →](docs/interrupt.md) | Universal interrupt vectors (pause / redirect); multi-modal real-time interruption; session idle timeout. |
| **Trace & observability** | System monitoring + logging: structured event log, real-time metrics | 15 event types streamed via EventBus → `FileTraceStore` (JSON per session) + `UsageTracker` (token stats). Frontend waterfall view grouped by interaction round, expandable to per-tool detail. SSE live stream. [Design doc →](docs/trace.md) | SQLite-based trace DB; OpenTelemetry export; trace search/filter; span tracing with parent-child correlation. |
