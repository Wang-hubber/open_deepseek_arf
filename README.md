...
| **Model routing & resource allocation** | Multi-level cache (L1/L2) + big.LITTLE heterogeneous scheduling | Two-tier LLM classifier routes user tasks by complexity (medium→quick, complex→deep). Dedicated model handles framework background work (memory extraction, classification). Per-turn dynamic switching. [Design doc →](docs/model-routing.md) | Hardware-aware dispatch: task complexity × latency budget × KV cache occupancy; same-session transparent model switching; weak-model ensemble exceeding single strong model |
