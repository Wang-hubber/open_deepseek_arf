...
|---------|-------------|---------------|-----------|
| **Context window exhaustion (OOM)** | Virtual memory + page swapping: evict cold pages to disk, fault in on demand | `SlidingWindowCompactor` monitors total character count per turn, triggers at threshold (75% of 1M token window). Retains last 4 messages, summarizes older turns via LLM into `context_summary`. Memory retrieval results merged into same summary field. [Design doc →](docs/compaction.md) | Fine-grained "page fault": retrieve and inject minimal semantic units (function signatures, key decisions) rather than whole context blocks; LRU eviction by relevance |
