...
| **External interrupt & user intervention** | Hardware interrupt: save state → ISR → restore state | `asyncio.Event` cancellation token checked per-turn. `POST /api/chat/cancel`, client disconnect, and Hook exit-code-2 injection. Frontend stop button triggers server-side cancel. [Design doc →](docs/interrupt.md) | Universal interrupt vectors (pause / redirect / undo / inject); multi-modal real-time interruption; session idle timeout. |
