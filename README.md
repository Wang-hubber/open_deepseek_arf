...
| **External capability invocation** | System calls + coprocessors: CPU invokes NPU/GPU/disk via interrupt + I/O | Tool calling: `tool.yaml` (JSON Schema) + `function.py` per tool, loaded via dual-source registry. Subprocess hooks with exit-code contract (0=continue, 1=block, 2=inject). MCP protocol support in roadmap. | Hardware-accelerated tool execution: high-frequency tools (code interpreter, search) wrapped as standard "coprocessor" interfaces; parallel batch dispatch with DMA-style async I/O |
