...
| **Trace & observability** | System monitoring + logging: structured event log, real-time metrics | 13 event types via EventBus → `FileTraceStore` (JSON) + `UsageTracker`. Frontend waterfall grouped by interaction round, expandable: model response → tool calls → hooks. Resource stats API. Standalone viewer at `/trace-viewer`. [Design doc →](docs/trace.md) | SQLite trace DB; OpenTelemetry export; trace search/filter; span tracing. |
