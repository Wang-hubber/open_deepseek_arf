...
| | **Security** | `PathCheckToolGuard` (.., symlink, depth/count), `DirectoryBoundary` whitelist elevation, `SessionModeManager` (auto/ask/plan) + `PermissionRegistry` deny→ask→allow, `HumanLoop` SSE approval + 60s timeout, `GuardDefaults` three-line defense, `ContentGuard` dangerous_patterns (block) + sensitive_patterns (redact), `SandboxManager` (session sandbox isolation) |
...
`ContentGuard` adds two additional guard layers: `dangerous_patterns` blocks known attack patterns (prompt injection, command injection, code injection) and `sensitive_patterns` redacts credentials, API keys, and PII from tool arguments and model output — checked at 3 checkpoints (pre-execution, post-model-response, post-tool-output).
    ...
    content_guard:
      ...
      dangerous_patterns:
        - type: prompt_injection
          pattern: "ignore previous instructions|forget all rules"
        - type: command_injection
          pattern: "[;&|]\\s*(rm|sh|bash|powershell)"
      sensitive_patterns:
        - type: api_key
...
`SandboxManager` provides session-level workspace isolation. On session start, the workspace is copied to `sandbox/{session_id}` (excluding blacklisted paths). Tools execute against the sandbox copy; at round end, a diff reveals all file changes, pending approval before persistence back to the workspace. Sandbox directories are destroyed with `auto_destroy` or cleaned manually.
