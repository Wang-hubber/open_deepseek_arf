...
| **[Tool Sandbox →](docs/tool-sandbox.md)**<br>Security boundaries | System calls + protection rings (Ring 0–3) + ACL | `tool.yaml` + `function.py` per tool. `PathCheckToolGuard` recursive scan blocks traversal, symlinks, depth/count quota. Permission deny→ask→allow pipeline. | Per-invocation sandbox; MCP protocol |
