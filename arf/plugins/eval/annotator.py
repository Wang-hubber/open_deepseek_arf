...
DEFAULT_OUTPUT_CONTAINS_PROMPT = (
    "你是一个精确的评测标注员。阅读下面的对话，提取 **2–5 个关键词** "
    "来描述用户期望的 agent 输出。\n\n"
    "规则：\n"
    "1. 只提取关键词，不要完整句子\n"
    "2. 关键词应该是中文，匹配 agent 最终输出中应出现的词汇\n"
    "3. 不要包含工具名、参数名\n"
    "4. 只输出一个 JSON 字符串数组，不要其他内容\n\n"
    "示例输出格式：[\"关键词1\", \"关键词2\"]\n\n"
    "---\n"
    "用户输入：{input}\n\n"
    "Agent 最终输出：{original_output}\n\n"
    "Agent 调用的工具：{tool_names}\n"
    "{context_summary}"
    "{feedback}"
)

DEFAULT_EXECUTION_PROMPT = (
    "你是一个精确的评测标注员。阅读下面的对话，提取 agent 在处理该请求时"
    " **应该调用** 的工具名列表（按调用顺序）。\n\n"
    "规则：\n"
    "1. 只输出一个 JSON 字符串数组，不要其他内容\n"
    "2. 工具名必须精确匹配实际工具名称\n"
    "3. 只列出必需的工具，不要列出可选的\n\n"
