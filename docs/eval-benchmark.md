...
真实对话 → TracePlugin → data/{sid}/traces/{sid}.jsonl
                           ...
                           BenchmarkBuilder.build(session_id, name)
                                    │
                                    ▼
                           benchmarks/{name}.json (评测合约，含 expected_execution/expected_reasoning/expected_output_contains/max_turns + feedback)

  Case 边界：按 user_input 在 JSONL 中的位置索引切分，不依赖 turn 号。
  第 i 个到第 i+1 个 user_input 之间的所有事件属于 case_i。
  user_annotation 事件按 source_round 关联到对应 case。
                                    │
                                    ▼
                           EvalRunner
...
# 从真实对话 session 创建 benchmark（自动提取 expected_execution）
benchmark = builder.build(session_id="default", name="file_ops_v1")

# annotate_mode=True：语义字段填入占位符，引导后续标注
benchmark = builder.build(session_id="default", name="file_ops_v1",
                          annotate_mode=True)
...
```json
{
  ...
  "trace_snapshot_path": "file_ops_v1.trace.jsonl",
  "cases": [
    {
      "id": "case_0",
      ...
      "source_round": 1,
      "expected_reasoning": [
        "识别用户要读取 README.md 文件",
        "使用 read 工具读取文件内容"
      ],
      "expected_execution": [
        {"type": "tool", "name": "read", "params": {"path": "README.md"},
         "result_preview": "# ARF Framework\n\nARF 是一个...", "success": true}
      ],
      ...
      "feedback": {
        "rating": "thumbs_up",
        "reason": "正确读取了文件",
        "annotated_at": "2026-06-15T10:30:00"
      }
    }
  ]
}
...
| `id` | `str` | — | 用例 ID |
| `input` | `str` | — | 用户消息文本 |
| `session_id` | `str \| None` | `None` | 来源 trace session，LLM metrics 通过它读取完整 trace 做 reference 评测 |
| `source_round` | `int \| None` | `None` | 来源 session 中的第几个 round |
| `expected_reasoning` | `list[str]` | `[]` | 预期推理链条，LLM-Judge 对比实际 reasoning |
| `expected_execution` | `list[dict]` | `[]` | 预期执行链条，每条 `{"type": "tool", "name": "...", "params": {...}}` |
| `expected_output_contains` | `list[str]` | `[]` | 预期输出包含的关键词（子串匹配） |
| `max_turns` | `int \| None` | `None` | 预期最大 turn 数 |
| `feedback` | `dict \| None` | `None` | 运行时人工标注：`{"rating": "thumbs_up\|thumbs_down", "reason": "...", "annotated_at": "..."}` |

`expected_execution[i]` 结构：`{"type": "tool", "name": "read", "params": {"path": "x"}, "success": true, "result_preview": "..."}`。`type` 固定为 `"tool"`，`params`、`success`、`result_preview` 均可选——不标则该维度不参与匹配。

`feedback` 由 `BenchmarkBuilder` 从 trace 中的 `user_annotation` 事件自动提取（同 round 多条取 timestamp 最新）。无标注时为 `None`。

*已移除字段：`expected_tools`、`expected_tool_calls`（被 `expected_execution` 替代）、`golden_trajectory`、`original_output`。*
...
| `metrics` | 8 维开关 dict |
| `prompts` | `dict[str, str]` | 覆盖 LLM 指标的 prompt（key: `tool_call_result_llm`/`output_quality`/`output_quality_free`/`trajectory_similarity`/`trajectory_similarity_free`/`reasoning_similarity`），不传用内置默认 |
| `mode` | `"online"` / `"offline"` |
...
| `tool_call_accuracy` | `float` | 工具调用准确率均值 |
| `execution_accuracy` | `float` | 执行准确率均值（规则化，对比 expected_execution vs actual） |
| `reasoning_similarity` | `float \| None` | 推理相似度均值（LLM-Judge 1-5 分，无 judge 或空预期时 None） |
| `output_quality` | `float \| None` | 输出质量均值（LLM-Judge 1-5 分） |
| `trajectory_similarity` | `float \| None` | 轨迹相似度均值（LLM-Judge 1-5 分） |
...
| `ToolCallAccuracyMetric` | 从 `expected_execution` 过滤 `type="tool"` 条目，按名称 + params 子集匹配。同步统计 dependency_order_failures | 0–1 + dep_fail 计数 |
| `ExecutionAccuracyMetric` | 同 ToolCallAccuracyMetric 的匹配逻辑，但作为独立指标——对比 `expected_execution` vs actual 全部 tool calls | 0–1 |
| `TurnEfficiencyMetric` | 实际 turn 数 vs `max_turns` | 0–1 |
| `OutputContainsMetric` | 实际最终输出是否包含 `expected_output_contains` 所有关键词（子串匹配） | 0–1 |

**ToolCallAccuracyMetric 与 ExecutionAccuracyMetric 的区分**：两者共享相同的 name + params 子集匹配逻辑。`ToolCallAccuracyMetric` 是针对历史 `expected_tools`/`expected_tool_calls` 的演进——现统一从 `expected_execution` 读取。`ExecutionAccuracyMetric` 是语义层指标——即使工具名称和参数都正确，执行结果（success/failure）可能不同，该指标仅判断调用本身是否正确。

**ToolCallAccuracyMetric 匹配策略**：

1. 从 `expected_execution` 过滤 `type="tool"` 条目，按**名称**与 actual 配对（不关注执行顺序）
2. 每个 expected entry 在 actual 中找同名的、params 子集匹配的，找到即算命中
...
6. `expected_execution` 为空时返回 1.0
...
| `ReasoningSimilarityMetric` | LLM 对比 `expected_reasoning` vs actual model reasoning，1-5 打分。可降级——无 judge 或空预期时返回 N/A | score + reason |

**ToolCallResultLLMMetric** 用于评估工具**返回值**的语义一致性。`ToolCallAccuracyMetric` 负责名称和参数（程序化、零开销），`ToolCallResultLLMMetric` 负责结果语义（需 judge LLM）。`result_preview` 可从 golden trajectory 自动提取，人工标注是可选的优化——当 golden result 太冗长时，人工可以改成松散的语义描述让 LLM 判得更准。推荐先跑程序化 metric，只在需要时开启 LLM 裁判。
...
**裁判 prompt 默认值在 `arf/plugins/eval/plugin.yaml` 的 `config.prompts` 下集中管理**，六个 key：
...
- `reasoning_similarity` — 对比 `expected_reasoning` vs actual model reasoning，1-5 打分
...
`BenchmarkBuilder.build()` 自动从 trace 提取 `expected_execution`（含 `type="tool"` + name + params + result_preview）。被安全策略阻止的工具调用（`success: false`）也会被包含——这是正确行为的一部分。产出的 benchmark JSON 可作为起点，人工标注做三件事：

1. **删**：移除不关键的 turn（如中间探索性的 glob）
2. **改**：修正 `expected_output_contains` 预期关键词，缩紧 `max_turns`
3. **标**：填写 `expected_reasoning` 推理链条定位，给关键 tool call 写 `params` 约束
4. **查**：需要完整上下文时查看 trace 快照文件 `{benchmark_dir}/{name}.trace.jsonl`

`annotate_mode=True` 时 Builder 自动填入占位符（`[待标注] 该轮预期推理步骤...`），引导后续标注流程。
...
```json
{
  ...
  "input": "良子去吃良子的焖子",
  "expected_execution": [
    {"type": "tool", "name": "eat", "params": {"name": "良子", "path": "良子的焖子"}}
  ],
  "expected_reasoning": [
    "理解用户想吃良子的焖子",
    "调用 eat 工具"
...
```json
{
  ...
  "input": "查一下今天天气",
  "expected_execution": [
    {"type": "tool", "name": "weather", "params": {"city": "北京"}, "result_preview": "晴，22°C"}
  ]
}
```

`result_preview` 走 LLM 语义等价判断——"晴天，气温 22 摄氏度" 也能匹配 "晴，22°C"。

**带 success 预期（验证安全策略或工具执行状态）：**
...
```json
{
  ...
  "expected_execution": [
    {"type": "tool", "name": "read", "params": {"path": "/etc/passwd"}, "success": false}
  ],
  "expected_reasoning": [
    "用户请求读取系统文件 /etc/passwd",
    "安全策略应拦截此操作"
  ]
}
```

`success` 在 `ToolCallAccuracyMetric` 中走程序化匹配（不走 LLM），仅在标注后生效。
...
- **多轮对话**：每轮（一个 user input → 最终 text response）一个 EvalCase。如果一轮中有多个 tool_call，全放在同一个 `expected_execution` 数组里
- **params 标关键字段即可**：不用标全量参数，标对决策有影响的字段（如 `path`、`pattern`、`name`）。框架自动注入的参数（`_workspace`）不要标
- **expected_reasoning 填推理链条**：按步骤描述 agent 应该怎么想，用于 `ReasoningSimilarityMetric`（LLM-Judge）。不需要详尽——关键判断点即可
- **result_preview 可标可不标**：不标时 `ToolCallResultLLMMetric` 自动跳过（返回 1.0）。如需 LLM 裁判结果语义，写 "返回了用户列表 JSON" 而非原始返回值全文
- **反馈标注**：`feedback` 字段由 Builder 从 trace `user_annotation` 事件自动提取，无需人工填写
...
```
 ...
 Cases: 5 to run
  ...
  [OK] case_0: turns=1, tok=120/85, tool_acc=1.00, turn_eff=1.00, exec_acc=1.00, quality=4/5, reason_sim=4/5, 2.3s
  [OK] case_1: turns=1, tok=150/96, tool_acc=1.00, turn_eff=1.00, exec_acc=1.00, quality=5/5, 3.1s
  [FAIL] case_2: turns=2, tok=310/178, tool_acc=0.50, turn_eff=1.00, exec_acc=0.50, quality=3/5, 4.2s
         input: 帮我读一下 README.md 和 config.yaml
  [OK] case_3: turns=1, tok=95/44, tool_acc=1.00, turn_eff=0.50, exec_acc=1.00, 2.8s
  [OK] case_4: turns=2, tok=220/130, tool_acc=1.00, turn_eff=1.00, exec_acc=1.00, quality=5/5, traj_sim=5/5, reason_sim=5/5, 1.9s
...
config = EvalConfig(
    ...
    metrics={
        "tool_call_accuracy": True,
        "tool_call_result_llm": False,       # LLM，需 judge + judge_model + 标注了 result_preview
        "turn_efficiency": True,
        "success_rate": True,
        "execution_accuracy": True,          # 规则化，对比 expected_execution vs actual
        "reasoning_similarity": False,       # LLM，需 judge + judge_model + expected_reasoning
...
# 2. 人工编辑 JSON — 标注 expected_reasoning / expected_output_contains
#    需要完整上下文时查看 benchmarks/file_ops_v1.trace.jsonl（快照）
...
Trace JSONL → Builder 自动提取 → 人工精修 JSON → 运行 Eval
                ↓                        ↓
         expected_execution       expected_reasoning 填写
         (自动，含 result_preview) expected_output_contains 填写
         feedback 自动提取       
