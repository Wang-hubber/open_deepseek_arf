...
大型语言模型推理能力的跃升推动了 Agent 范式的快速普及。两个落地领域获得了显著关注：其一为数字世界中的 CLI 与编程辅助，其二为真实世界中的视觉-语言-动作（VLA）系统。两种范式并行发展——垂直领域专精路径与全域通用路径——无论哪种，Harness（运行框架）都已成为实际部署中无法绕过的组件。
...
> **TODO**：绘制 Agent 架构图，标注 RAG、提示词构造、记忆读取、摘要压缩在 Harness 层的分布位置，标识"认知泄漏点"。
...
当前实践的深层驱动力或许是一种补偿式工程思维——用 Harness 弥补基座模型不足，而非将模型能力边界作为训练目标来推进。

> **本节 TODO**：绘制数据流图（ASCII 或 SVG），标注认知泄漏点；从现有框架提取 3–5 个典型代码案例。
...
**4.2 脊椎 = Harness：硬线 + 软线** ← **核心理论贡献**

部分神经科学文献将脊椎描述为双重功能结构：传导运动指令的下行通路（硬线），以及承载感觉信号上行整合的后索通路（软线）。借用这一划分，Agent 的 Harness 或许同样由两条并行线构成——一条零认知且永不变更，另一条承载认知信号的渐进内化。
...
前四个实验各自验证单一维度的参数化迁移效果。实验五将所有维度叠加——Identity + Knowledge + Memory + Comm LoRA 同时激活——在统一的硬线上对比两种软线配置的终局表现。
...
- [ ] **绘制 §3.1 数据流图**：ASCII 或 SVG 图，标注认知功能在 Harness 层的分布及"认知泄漏点"
