...
> Phase 3 — AgentConfig 第二项任务
> 父文档：`docs/v1.x/phase3_agent_config/phase3-agent-config-design.md`
> 依赖：3.1（`ModelSpec` + `ToolSpec` + `ToolPermission`，已完成）

## 设计思路

`ResourceSpec` 是 Agent 声明"我需要什么类型的资源"的结构体。它不引用具体的 `NodeId`，只描述资源的**逻辑特征**。Engine（Phase 4）在 Bus 上做 discovery 时，用 `ResourceSpec` 的 `node_type` + `capabilities` 去匹配在线的节点。

| 字段 | 用途 | 示例 |
|------|------|------|
| `name` | Agent 给资源起的别名 | `"primary_fs"`, `"code_reviewer"` |
| `node_type` | Bus 上的节点类型 | `"mcp"`, `"agent/subagent"`, `"agent/teammate"` |
| `capabilities` | 可选的能力匹配条件 | `{"resources": ["tool/read", "tool/write"]}` |

1:N 语义：一个 `ResourceSpec` 可能匹配 Bus 上多个节点（如 3 个 `mcp/filesystem` 同时在线）。Engine 全部注册到 `ResolvedResource.nodes: Vec<NodeId>`，运行时选第一个在线的。

## 代码实现

### `crates/arf-agent/src/resource.rs`（新文件）
...
逐行：
- `name: String` — Agent 自己取的别名，不参与 Bus 匹配，只用于日志和错误信息。同一个 Agent 可以声明 `"primary_fs"` 和 `"backup_fs"` 两个 filesystem 资源
- `node_type: String` — 匹配 Bus 上 `NodeInfo.node_type`。不是 enum 因为 node_type 是开放集合（应用可自定义新类型）
- `capabilities: Option<serde_json::Value>` — `None` = 匹配所有同 node_type 的节点。`Some` 时做子集匹配：节点的 capabilities 需包含此处声明的所有 key/value
- `#[serde(default, skip_serializing_if = "Option::is_none")]` — 无能力过滤要求时不输出到 JSON

---

### `crates/arf-agent/src/lib.rs` — 追加

在 `pub use tool::{ToolPermission, ToolSpec};` 之后追加一行：
...
所有测试在 `crates/arf-agent/src/resource.rs` 的 `#[cfg(test)] mod tests` 块内。
...
| 类型 | 测试数 | 覆盖角度 |
|------|--------|---------|
| ResourceSpec | 9 | 构造(×2：全字段+无capabilities)、边界(×3：空name、capabilities跳过、最小JSON)、Clone、PartialEq、序列化(×2：全字段+最简) |
| **累计** (3.1 + 3.2) | **34** | |
...
- 新增 `crates/arf-agent/src/resource.rs`
- 修改 `crates/arf-agent/src/lib.rs`（加 `mod resource;` + `pub use`）
- 不修改其他文件
...
- `cargo test --workspace` 全部通过（含已有 212 + 新增 9 = 221 tests）
- `cargo fmt --check` + `cargo clippy` 无警告
- `ResourceSpec` serde 往返一致，capabilities 可选
- `arf-agent` 仅依赖 `serde` + `serde_json`
