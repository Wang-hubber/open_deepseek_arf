...
pub struct Task {
    ...
    /// (local task, external tasks it depends on) pairs.
    /// Stored as `Vec` rather than `HashMap` because JSON object keys must be strings
    /// and `TaskId` is a compound type.
    pub blocked_by: Vec<(TaskId, Vec<TaskId>)>,
    /// (local task, external tasks that depend on it) pairs.
