...
impl Engine {
    ...
    /// 2. 循环（每 turn）：
    ///    a. model_call → model_response（content + tool_calls）
    ///    b. 推 assistant message
    ///    c. 若 content 无 tool_calls → 终止，返 content
    ///    d. 否则依次 tool_exec → tool_result（推 tool message）
    /// 3. 终止条件（任一满足即返）：
    ...
    async fn do_model_turn(
        ...
        eprintln!("[do_model_turn] got response");
    ...
    async fn send_and_await(
        ...
        self.wait_for_response_matching(cid).await
    }
    ...
    async fn wait_for_response_matching(
        ...
        loop {
            ...
            // Skip non-response message types
            if msg.msg_type != MODEL_RESPONSE && msg.msg_type != TOOL_RESULT {
                continue;
            }
...
fn engine_response_types(config: &AgentConfig) -> Vec<String> {
    ...
    if config.routes.contains_key("tool_exec") {
        types.push("tool_result".into());
    }
    types.sort();
    types.dedup();
    types
